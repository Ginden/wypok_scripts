...
function main() {
    ...
    function getTimeBasedUniqueString() {
        ...
        var everyHours = Math.max(1, +(/\d*/.exec(settings.CACHE_REFRESH_TIME))); // wyciąga cyfry, wspieramy tylko
                                                                                  // liczby godzin.
...
function template(code, values) {
    ...
    return code.replace(/!\{(.*)}/gi, function(group, value){
    ...
    if(settings.ENHANCED_BLACK_LIST_LINKS) {
        $('#itemsStream .link').filter(function (i, el) {
            ...
            if (isBlockedAuthor) {
                ...
                blockedTag = find(tags, blockedTags.has.bind(blockedTags))
                if (blockedTag) {
    ...
    getReports(function (reports) {
        ...
        getModerated(function (moderated) {
            moderated.forEach(function (el) {
                var $li = $(template(TEMPLATES.MODERATED_LI, {
                    url: el.url,
                    reason: el.reason,
...
var scriptVersion = typeof GM_info !== 'undefined' ? GM_info.script.version : '5.10';
